...
export function validateBody(body, allowed) {
  ...
  for (const key of Object.keys(body))
    if (!allowed.includes(key)) fail(`不支持的字段：${key}`);
}
function readString(value, name, max = 300) {
  if (typeof value !== "string" || value.length > max)
    fail(`${name}必须为不超过 ${max} 字的文字`);
  return value.trim();
}
...
function applyBasics(job, body) {
  for (const field of basics)
    if (field in body)
      job[field] = readString(
        body[field],
        field,
        field === "notes" ? 20000 : field === "url" ? 2048 : 300,
      );
...
function validateAction(job) {
  if (!["none", "deadline", "appointment"].includes(job.dateType))
    fail("时间类型无效");
  validDate(job.date);
...
function event(job, old, now, title, text, outcome) {
  const previousRoundRecord = old.rounds.find(
    (r) => r.stage === old.stage && r.round === old.round,
  );
  const roundRecord = job.rounds.find(
    (r) => r.stage === job.stage && r.round === job.round,
  );
  job.history.unshift({
    id: randomUUID(),
    title,
    ...
    outcome,
    ...(previousRoundRecord
      ? { previousRoundRecord: structuredClone(previousRoundRecord) }
      : {}),
    ...(roundRecord ? { roundRecord: structuredClone(roundRecord) } : {}),
  });
}
export function editJob(old, body, now) {
  ...
  event(
    ...
    "编辑求职记录",
    old.action
      ? `${cancelled ? "原行动已取消" : "原行动"}：${old.action}${old.date ? `；原时间：${old.date}` : ""}`
      : "已更新基础资料。",
...
const stages = {
  application: ["待投递", "已投递"],
  assessment: ["待安排", "待完成", "待反馈"],
  interview: ["待安排", "待完成", "待反馈"],
};
const results = {
  offer: "收到 Offer",
  rejected: "未通过",
...
function saveRound(job, body) {
  ...
  if (job.date && job.status === "待完成") round.date = job.date;
  if ("roundNotes" in body)
    round.notes = readString(body.roundNotes, "轮次备注", 20000);
  if ("roundResult" in body)
    round.result = readString(body.roundResult, "轮次结果", 2000);
}
export function commandJob(old, body, now) {
  ...
  if (body.type === "progress") {
    ...
    job.round =
      "round" in body
        ? readString(body.round, "轮次")
        : job.stage === old.stage
          ? old.round
          : "";
    for (const field of ["roundNotes", "roundResult"])
      if (field in body)
        readString(body[field], field, field === "roundNotes" ? 20000 : 2000);
    if (
      (body.roundNotes || body.roundResult) &&
      (!job.round || !["assessment", "interview"].includes(job.stage))
    )
      fail("请为测评或面试轮次填写轮次名称");
    for (const field of ["action", "date", "dateType"])
      if (field in body) job[field] = readString(body[field], field);
    const completing =
      ["assessment", "interview"].includes(job.stage) &&
      job.status === "待反馈" &&
      (job.stage !== old.stage || old.status !== "待反馈");
    if (completing) clearAction(job);
    validateAction(job);
    saveRound(job, body);
    title = `进展更新：${stageNames[old.stage]} · ${old.status} → ${stageNames[job.stage]} · ${job.status}`;
    outcome =
      completing && old.action
        ? "completed"
        : old.action !== job.action ||
            old.date !== job.date ||
            old.dateType !== job.dateType
          ? job.action
            ? "updated"
            : "cancelled"
          : "status-updated";
    text = old.action
      ? `${outcome === "completed" ? "行动已完成" : "原行动"}：${old.action}${old.date ? `；原时间：${old.date}` : ""}`
      : "已记录阶段和轮次进展。";
    ...
    if (
      job.stage !== "result" ||
      job.resultType !== "offer" ||
      job.offerDecision !== "pending"
    )
      fail("当前没有待决定的 Offer");
    ...
    clearAction(job);
    title =
      body.decision === "accepted" ? "记录已接受 Offer" : "记录已婉拒 Offer";
    text = old.action
      ? `原行动：${old.action}${old.date ? `；原时间：${old.date}` : ""}`
      : "已保存个人决定。";
    outcome = "offer-decided";
  } else {
    if (body.type === "archive" && (job.stage !== "result" || job.archived))
      fail("仅未归档的结果记录可以归档");
    if (body.type === "restore" && !job.archived) fail("记录尚未归档");
    job.archived = body.type === "archive";
    title = job.archived ? "归档求职记录" : "恢复求职记录";
    text = "保留原有阶段、结果及行动。";
    outcome = body.type;
  }
  if (job.stage !== old.stage || job.status !== old.status)
    job.statusSince = now;
  if (!job.appliedAt && job.stage === "application" && job.status === "已投递")
    job.appliedAt = new Date(Date.parse(now) + 8 * 3600000)
      .toISOString()
      .slice(0, 10);
