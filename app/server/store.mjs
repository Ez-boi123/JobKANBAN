    ...
    try {
      if (id) {
        if (!Number.isSafeInteger(version) || version < 1)
          throw new HttpError(400, "请提供记录版本");
        const row = db
          .prepare("SELECT version,data FROM jobs WHERE id=?")
          .get(id);
        if (!row) throw new HttpError(404, "未找到求职记录");
        if (row.version !== version)
          throw new HttpError(409, "记录已被更新，请刷新后重试");
        job = transform(JSON.parse(row.data));
      }
      db.prepare(
        "INSERT INTO jobs(id,version,data) VALUES(?,?,?) ON CONFLICT(id) DO UPDATE SET version=excluded.version,data=excluded.data",
      ).run(job.id, job.version, JSON.stringify(job));
      for (const event of job.history)
        db.prepare(
          "INSERT OR IGNORE INTO history(id,job_id,data) VALUES(?,?,?)",
        ).run(event.id, job.id, JSON.stringify(event));
