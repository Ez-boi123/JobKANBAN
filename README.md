...
打开 **提醒设置 → 修改邮件配置**，连接原服务所属的 Cloudflare 账户后，可更新发信邮箱、客户端授权码和收件邮箱。只改收件邮箱时，可以保留发信设置；授权码留空表示保留原值，更换发信邮箱时必须填写对应的新授权码。原授权码不会回显。

保存会先关闭邮件并同步撤销待发安排，再更新原 Worker，不会新建服务或更换同步密钥。成功后邮件仍保持关闭，请实际收到测试邮件后再开启。修改失败可在同一入口重试；云端配置变更未确认完成时，会阻止重新开启邮件。已进入发送流程或交给发信服务的邮件无法保证撤回。

此入口不用于更换 Cloudflare 账户或迁移服务地址。旧版手工部署需保留含原账户和服务名的 `reminder-worker/wrangler.local.jsonc`，详见部署指南。

![修改已有邮件配置（虚构账户与邮箱）](docs/images/reminder-edit.png)
